...
| 子資料夾 | 服務 | Port | 技術 |
|----------|------|------|------|
| `main/` | 中控 API（轉接 ASR / TTS、管理前端播放佇列） | 8080 | Python 3.12 + FastAPI |
...
|------|------|
| Python | 3.12 |
| 相依服務 | ASR（:8000）、TTS（:9999）——只測佇列端點時可不啟動 |

## 需要安裝的套件（見 [main/requirements.txt](main/requirements.txt)）
...
| 變數 | 預設值 | 說明 |
|------|--------|------|
| `ASR_API_URL` | `http://localhost:8000/transcribe` | ASR 服務位址 |
...
> 前端是純 Node.js 專案，**不需要 Python 虛擬環境**。
...
1. `asr`（:8000）與 `tts`（:9999）→ 見專案根目錄各自資料夾的 INSTALL.md
2. `main`（:8080）
3. `web`（:5173）
